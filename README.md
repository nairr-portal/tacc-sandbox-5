...
3. **Trains and predicts, with self-debug** — the LLM is given the task instruction, dataset preview, and a domain-knowledge hint (see below), and must binarize the continuous `Signal-inhibition` training target, train a `RandomForestClassifier`, and predict activity for the held-out test compounds. Its generated code is actually executed and its output checked; on a crash, a missing output file, or a wrong output schema, the error is fed back to the model for another attempt, up to 10 retries (see below).
...
This notebook starts from ScienceAgentBench's **self-debug** condition (`agent.py`'s `use_self_debug=True`) — the paper's own best-performing technique, reported to roughly double direct-prompting's success rate — and extends it by one check. Instead of one LLM call, the generated code is executed; each round checks whether it (a) ran without raising an exception, (b) actually wrote `pred_results/dkpes_test_pred.csv`, and (c) that file has the required `index`/`Signal-inhibition` columns. On failure, the error is fed back as a 3-turn context (system prompt, most recent code, new error) — the same bounded context the official implementation uses — and the model gets another attempt, up to 10 retries.

**Check (c) is our own addition, not part of the published technique** — the real `agent.py` self-debug loop only checks (a) and (b); it never re-runs the eval script's own checks mid-loop, only once at the end for scoring. We added the schema check after this notebook's first manual run: the model wrote a `predicted_signal_inhibition` column instead of `Signal-inhibition` — a defensible naming choice in isolation (avoids collisions when later merging predictions against ground truth), and one the official self-debug baseline's narrower checks wouldn't have caught, since the file existed and the code ran cleanly. We reasoned about this explicitly before extending the loop: telling the model the exact required column name in the prompt would answer the test on the precise failure mode self-debug exists to fix, so we didn't; but ScienceAgentBench's own paper isn't the goal here, building working notebooks is, so extending the technique itself was the better fit for this project's own goals. The check is deliberately scoped as a *submission-format* validator, not a correctness check — it reads only the predicted file's column headers, never the predicted values or the withheld gold labels, so the model never gets test-set feedback through this loop. Validated the loop mechanics against a mocked LLM before publishing, including the exact real failure sequence (crash → wrong column name → correct schema, confirming the extension catches and recovers from precisely the bug this notebook hit) — the test script itself is a scratch artifact, not part of this repo.
